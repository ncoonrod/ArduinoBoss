...
dotnet run -- --code 12345 --name Aria
...
1. Opens the serial port to the Arduino at 9600 baud and reads `BTN:PRESS` events.
2. POSTs `/v1/game/join` with the lobby code + name, then `/v1/game/set_arduino_status` with `connected: true`.
...
5. On `BTN:PRESS` from the Arduino, calls `report_hit` (if mid-attack) or `report_early_press` (otherwise).
...
| File | Mirrors |
|------|---------|
| `ArduinoSerial.cs` | `public/js/serial.js` |
| `GameApi.cs` | `public/js/api.js` |
| `GameWebSocket.cs` | `public/js/ws.js` |
| `Models.cs` | `src/GameState.ts` interfaces |
| `Program.cs` | `public/js/game.js` (player flow) |

## Notes

- The browser client uses Web Audio + on-screen UI; this console client uses only the Arduino LED/buzzer and stdout. The block UX is identical though — the Arduino's tactile button is the input.
